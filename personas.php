...
if (!perfil_valido(1)) {
	...
	if (!$rs) {
		...
		foreach($rs as $fila) {
			...
			$cargo=utf8_encode($fila['desc_usuario']);			
			//$agregarTrabajo=" <a href='personas_cargo.php?tipo=M&id_cliente={$fila['id_cliente']}'> Modificar Trabajo</a> ";
		
			
			$nombre=utf8_encode($fila['apellido']).", ". utf8_encode($fila['nombre']);
			
			$lista.="<li>".
					"  <strong>$cargo</strong> --- ". 
					"  $nombre --- ".
					"  <a href='personas_abm.php?tipo=M&id_cliente={$fila['id_cliente']}'>Modificar</a> | ".
					"  <a href='#' onclick='javascript:borrar({$fila['id_cliente']});'>Baja</a> ".
					//"  $agregarTrabajo ".
					"</li>";
		}
...
<body>
    <div class="container">
        ...
        <main id="cuerpo">
            ...
            <a href="personas_abm.php?tipo=A">&raquo;Ingresar un usuario</a>
